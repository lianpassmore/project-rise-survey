...
export default async function handler(req, res) {
  ...
  try {
    if (type === 'listTools') {
      const resp = await aiSafetyServer.server.request(ListToolsRequestSchema, params || {});
      return res.status(200).json(resp);
    }
    if (type === 'callTool') {
      const resp = await aiSafetyServer.server.request(CallToolRequestSchema, params || {});
      return res.status(200).json(resp);
    }
